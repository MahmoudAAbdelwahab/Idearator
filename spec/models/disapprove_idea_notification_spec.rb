...
describe DisapproveIdeaNotification do
  before :each do
    ...
    @idea.user = @user
    @notification = DisapproveIdeaNotification.create(user: @user, idea: @idea, users: [@user])
  end

  describe 'send_notification' do

    it 'adds disapprove idea notification to idea_notifications table'do
      DisapproveIdeaNotification.send_notification(@user, @idea, [@user])
    ...
    it 'adds disapprove idea notification to notifications table' do
      expect(Notification.last.subtype).to eq ('DisapproveIdeaNotification')
    end
  ...
  describe 'text' do
    ...
    it 'returns the notifications text'do
      expect(@notification.text).to eq('Your idea idea1 wasn\'t approved by the committee.')
    end
  end
